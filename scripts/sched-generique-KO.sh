...
# Demarrage et arret des pompes piscine
#
# Pour la pompe principale (4 vitesses) : [DUMMY] : pas d'interface numerique avec la pompe, simulation via des virtual switches sous Domoticz
# 

## selon le nom de ce script, on deduit les 2 parametres, on active/desactive un des peripheriques

## non c'est mieux d'avoir une cron qui appelle un script different pour chaque action (plus facile a isoler si panne)

exit 0

sched-pompeV2-start.sh
sched-pompeV2-stop.sh
sched-pompeV4-start.sh
sched-pompeV4-stop.sh
sched-pompeV3-start.sh
sched-pompeV3-stop.sh
sched-polaris-start.sh
sched-polaris-stop.sh
sched-chlore-start.sh
sched-chlore-stop.sh
